...
export function registerRequestHeaders() {
    axios.interceptors.request.use(config => {
        if (!config.hasBaseUrl && !config.url.includes('/api/metersphere')) {
            if (config.url.startsWith('/')) {
                config.url = '/api/metersphere' + config.url;
            } else {
                config.url = '/api/metersphere/' + config.url;
            }
        }
...
export default {
    install(Vue) {
        ...
        function exception(error, result, url) {
            const errorCodeTipsList = [400, 500, 502, 404];
            ...
            if (error.response && errorCodeTipsList.includes(error.response.status)) {
                Message.error({ message: '网络连接异常,请重试', showClose: true });
                return;
            }
            result.loading = false;
