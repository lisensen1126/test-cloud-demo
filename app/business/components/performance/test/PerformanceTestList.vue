    ...
    <ms-container>
        <ms-main-container>
            <el-card class="table-card" v-loading="result.loading">
                ...
                >
                    <el-table-column prop="num" label="ID" width="100" show-overflow-tooltip>
                    </el-table-column>
                    <el-table-column prop="name" :label="$t('commons.name')" show-overflow-tooltip>
                        <template v-slot:default="scope">
                            <span @click="link(scope.row)" style="cursor: pointer;">{{
                                scope.row.name
                            }}</span>
                        </template>
                    </el-table-column>
    ...
    methods: {
        ...
        initTableData() {
            ...
            this.result = this.$post(
                this.buildPagePath('/performance/list'),
                this.condition,
                response => {
                    ...
                    this.tableData.forEach(test => {
                        this.result = this.$get(
                            '/performance/test/report-count/' + test.id,
                            response => {
