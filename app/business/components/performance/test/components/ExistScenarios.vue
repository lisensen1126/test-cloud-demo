<template>
    <el-dialog
        :title="$t('load_test.scenario_list')"
        :close-on-click-modal="false"
        :destroy-on-close="true"
        width="60%"
        :visible.sync="loadApiAutomationVisible"
        :modal="true"
    >
        <div style="width:100%">
            <el-table
                v-loading="projectLoadingResult.loading"
                class="basic-config"
                :data="apiScenarios"
            ...
            >
                ...
                <el-table-column prop="num" label="ID"></el-table-column>
                <el-table-column prop="name" :label="$t('load_test.scenario_name')"></el-table-column>
                <el-table-column
                    prop="tags"
                    min-width="120px"
...
export default {
    ...
    methods: {
        ...
        getProjectScenarios() {
            ...
            this.projectLoadingResult = this.$post(
                '/api/automation/list/' + this.currentPage + '/' + this.pageSize,
                condition,
                res => {
                    let data = res.data;
                ...
                response => {
                    ...
                    data.forEach(d => {
                        ...
                        threadGroups.forEach(tg => {
                            tg.options = {};
                            tg.urlcount = d.urlcount || 0,
                            tg.urlname = d.urlname || []
