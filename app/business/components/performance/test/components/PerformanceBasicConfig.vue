    ...
    <div v-loading="result.loading">
        ...
        <el-row type="flex" justify="start" align="middle">
            <ms-table-button
                ...
                :content="$t('load_test.load_exist_jmx')"
                @click="loadJMX()"
            />
            <ms-table-button
                :is-tester-permission="true"
        ...
        <el-table class="basic-config" :data="threadGroups.filter(tg=>tg.deleted=='false')">
            <el-table-column :label="$t('load_test.scenario_name')">
                <template v-slot:default="{row}">{{ row.attributes.testname }}</template>
            </el-table-column>
            ...
            <el-table-column :label="$t('load_test.thread_group')">
                <template v-slot:default="{row}">
                    <el-select
                        v-model="row.tgType"
                    ...
                    >
                        <el-option
                            v-for="tg in threadGroupForSelect"
                            :key="tg.tagName"
                            :label="tg.name"
                            :value="tg.testclass"
                        ></el-option>
        ...
        <el-table class="basic-config" :data="tableData">
            <el-table-column prop="name" :label="$t('load_test.file_name')"></el-table-column>
            <el-table-column prop="size" :label="$t('load_test.file_size')"></el-table-column>
            <el-table-column prop="type" :label="$t('load_test.file_type')"></el-table-column>
            <el-table-column :label="$t('load_test.last_modify_time')">
            ...
            <el-table-column :label="$t('commons.operating')">
                <template v-slot:default="scope">
                    <el-button
                        ...
                        :disabled="!scope.row.id || isReadOnly"
                        type="primary"
    ...
    methods: {
        getFileMetadata(test) {
            ...
            this.result = this.$get(this.getFileMetadataPath + '/' + test.id, response => {
                ...
                if (!files) {
                    Message.error({
                        message: this.$t('load_test.related_file_not_found'),
                        showClose: true,
                    });
                    return;
                }
        ...
        handleDeleteThreadGroup(tg) {
            this.$alert(
                this.$t('load_test.delete_threadgroup_confirm') + tg.attributes.testname + '？',
                '',
                {
                    confirmButtonText: this.$t('commons.confirm'),
                    callback: action => {
        ...
        threadGroupDisable(row) {
            return (
                this.threadGroups.filter(tg => tg.enabled == 'true').length === 1 &&
                row.enabled == 'true'
            );
        },
        tgTypeChange(row) {
        ...
        importCase(caseObj) {
            ...
            let jmxName =
                caseObj.name.substring(0, suffixIndex) + '_' + new Date().getTime() + '.jmx';
            let threadGroups = findThreadGroup(caseObj.xml, jmxName);
            threadGroups.forEach(tg => {
                tg.options = {};
                tg.urlcount = caseObj.urlcount || 0
                tg.urlname = caseObj.urlname || []
            });
