        ...
        <ms-main-container>
            <el-card class="table-card" v-loading="result.loading">
                ...
                <el-table
                    border
                    :data="tableData"
                    class="adjust-table test-content"
                ...
                >
                    ...
                    <el-table-column prop="name" :label="$t('commons.name')" show-overflow-tooltip>
                        <template v-slot:default="scope">
                            <span @click="handleEdit(scope.row)" style="cursor: pointer;">{{
                                scope.row.name
                            }}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="maxUsers" :label="$t('report.max_users')" />
                    ...
                    <el-table-column prop="tps" label="TPS"> </el-table-column>
                    <el-table-column
                        width="200"
                        prop="createTime"
                        sortable
                    ...
                    <el-table-column width="150" :label="$t('commons.operating')">
                        <template v-slot:default="scope">
                            ...
                            />
                            <ms-table-operator-button
                                :tip="$t('load_test.report.diff')"
                                icon="el-icon-s-operation"
                                @exec="handleDiff(scope.row)"
                                type="warning"
                            />
                            <ms-table-operator-button
                                :is-tester-permission="true"
...
export default {
    ...
    data() {
        return {
            ...
            buttons: [
                {
                    name: this.$t('report.batch_delete'),
    ...
    methods: {
        initTableData() {
            ...
            this.result = this.$post(
                this.buildPagePath('/performance/report/list/all'),
                this.condition,
                response => {
                    ...
                    this.tableData.forEach(report => {
                        if (report.status === 'Completed' && !report.maxUsers) {
                            this.result = this.$get(
                                '/performance/report/content/testoverview/' + report.id,
                                response => {
                                    this.$set(report, 'maxUsers', response?.data?.maxUsers);
                                    this.$set(
                                        report,
                                        'avgResponseTime',
                                        response?.data?.avgResponseTime
                                    );
