...
function parseChildren(nodeItem, item, isDisable) {
    ...
    if (item.stepDescription && item.stepModel === 'TEXT') {
        let descNode = _parseChildren(children, item.stepDescription, undefined, isDisable);
        if (descNode) {
            descNode.children = [];
            _parseChildren(descNode.children, item.expectedResult, undefined, isDisable);
        }
    } else {
...
export function tagChildren(node, resourceName, distinctTags) {
    ...
    children.forEach(item => {
        let isCaseNode =
            item.data.resource &&
            item.data.resource.indexOf(i18n.t('api_test.definition.request.case')) > -1;
