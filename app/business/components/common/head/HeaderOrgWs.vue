<template>
    ...
    >
        ...
        <el-submenu
            ...
            v-roles="['test_manager', 'test_user', 'test_viewer']"
        >
            <template v-slot:title
                >{{ $t('commons.workspace') }}:
...
export default {
    ...
    data() {
        return {
            organizationList: [{ name: this.$t('organization.none') }],
            workspaceList: [{ name: this.$t('workspace.none') }],
            currentUserInfo: {},
    ...
    methods: {
        initMenuData() {
            if (hasRoles(ROLE_ORG_ADMIN, ROLE_TEST_VIEWER, ROLE_TEST_USER, ROLE_TEST_MANAGER)) {
                this.$get(
                    '/organization/list/userorg/' + encodeURIComponent(this.currentUserId),
                    response => {
        ...
        query(sign, queryString) {
            if (sign === 'org') {
                this.organizationList = queryString
                    ? this.orgListCopy.filter(this.createFilter(queryString))
                    : this.orgListCopy;
            }
            if (sign === 'ws') {
                this.workspaceList = queryString
                    ? this.wsListCopy.filter(this.createFilter(queryString))
                    : this.wsListCopy;
            }
        },
