    ...
    <div v-loading="result.loading">
        <el-card class="table-card">
            ...
            >
                ...
                <el-table-column
                    :label="$t('api_test.environment.name')"
                    prop="name"
                ...
                <el-table-column :label="$t('api_test.environment.socket')" show-overflow-tooltip>
                    <template v-slot="scope">
                        <span v-if="parseDomainName(scope.row) != 'SHOW_INFO'">{{
                            parseDomainName(scope.row)
                        }}</span>
                        <el-button
                            size="mini"
                            icon="el-icon-s-data"
                ...
                <el-table-column :label="$t('commons.operating')">
                    <template v-slot:default="scope">
                        <ms-table-operator
                            @editClick="editEnv(scope.row)"
                            @deleteClick="deleteEnv(scope.row)"
                        >
        ...
        <el-dialog title="域名列表" :visible.sync="domainVisible">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button @click="domainVisible = false" size="mini">{{
                    $t('commons.cancel')
                }}</el-button>
                <el-button type="primary" @click="domainVisible = false" size="mini">{{
                    $t('commons.confirm')
                }}</el-button>
            </span>
        </el-dialog>
...
import {
    Environment,
    parseEnvironment,
} from '@/business/components/api/test/model/EnvironmentModel';
...
export default {
    ...
    methods: {
        ...
        exportJSON() {
            ...
            envs.map(env => {
                //不导出id和projectId和模块启用条件
                if (env.config) {
                    //旧环境可能没有config数据
                    let tempConfig = JSON.parse(env.config);
                    if (
                        tempConfig.httpConfig.conditions &&
                        tempConfig.httpConfig.conditions.length > 0
                    ) {
                        tempConfig.httpConfig.conditions = tempConfig.httpConfig.conditions.filter(
                            condition => {
                                //删除”模块启用条件“，因为导入导出环境对应的项目不同，模块也就不同，
        ...
        handleProjectChange() {
            //项目选择下拉框选择其他项目后清空“启用条件”,因为项目变了模块也就变了。
        ...
        parseDomainName(environment) {
            ...
            if (environment.config) {
                ...
                //旧版本没有对应的config数据,其域名保存在protocol和domain中
