    ...
    <div>
        <el-row>
            <el-col :span="10">
                ...
                <el-popover placement="right-end" title="示例" width="600" trigger="click">
                    <ms-code-edit
                        :read-only="true"
                        height="400px"
                        :data.sync="title"
                ...
                <el-popover placement="right-end" title="示例" width="200" trigger="click">
                    <ms-code-edit
                        :read-only="true"
                        height="200px"
                        :data.sync="robotTitle"
                        :modes="modes"
                        :mode="'text'"
                    />
                    <el-button icon="el-icon-warning" plain size="mini" slot="reference">
                ...
                >
                    ...
                    >
                        <template slot-scope="scope">
                            <el-select
                                ...
                                :placeholder="$t('organization.message.select_receiving_method')"
                                size="mini"
                                :disabled="!scope.row.isSet"
                    ...
                    <el-table-column label="webhook" min-width="20%" prop="webhook">
                        <template v-slot:default="scope">
                            <el-input
                                v-model="scope.row.webhook"
                                placeholder="webhook地址"
                                size="mini"
                                :disabled="!scope.row.isSet || !scope.row.isReadOnly"
                            ></el-input>
                        </template>
                    </el-table-column>
...
const noticeTemplate =
    requireComponent.keys().length > 0 ? requireComponent('./notice/NoticeTemplate.vue') : {};
    ...
    data() {
        return {
            ...
            title:
                ...
                '        <p>您所执行的 ${testName} 接口测试运行失败/成功<br/>\n' +
                '            请点击下面链接进入测试报告页面</p>\n' +
    ...
    methods: {
        ...
        handleAddTask(index, data) {
            if (data.event && data.userIds.length > 0 && data.type) {
                ...
                if (
                    data.type === 'NAIL_ROBOT' ||
                    data.type === 'WECHAT_ROBOT' ||
                    data.type === 'LARK'
                ) {
                    if (!data.webhook) {
