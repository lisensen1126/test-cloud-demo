<template>
    <el-card class="table-card" v-loading="result.loading">
        ...
        >
            <el-table-column
                prop="sortIndex"
                :label="$t('api_test.home_page.failed_case_list.table_coloum.index')"
                width="100"
                show-overflow-tooltip
            >
                <template v-slot:default="scope">
                    <span :class="scope.$index <= 2 ? 'primary-icon' : 'info-icon'">{{
                        scope.row.sortIndex
                    }}</span>
                </template>
            </el-table-column>
            ...
            >
                <template v-slot:default="scope">
                    <ms-tag
                        v-if="scope.row.caseType == 'apiCase'"
                        type="success"
                        effect="plain"
                        :content="
                    ...
                    />
                    <ms-tag
                        v-if="scope.row.caseType == 'scenario'"
                        type="warning"
                        effect="plain"
                        :content="
            ...
            <el-table-column
                prop="testPlan"
                :label="$t('api_test.home_page.failed_case_list.table_coloum.test_plan')"
            >
                <template v-slot:default="{ row }">
            ...
            <el-table-column
                prop="failureTimes"
                :label="$t('api_test.home_page.failed_case_list.table_coloum.failure_times')"
                width="100"
                show-overflow-tooltip
            />
        </el-table>
        ...
        search() {
            if (this.projectId) {
                this.result = this.$get(
                    '/api/faliureCaseAboutTestPlan/' + this.projectId + '/10',
                    response => {
                        // console.log('过去7天测试计划失败用例TOP 10',response)
