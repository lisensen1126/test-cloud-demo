    ...
    <div v-loading="isReloadData">
        <el-row>
            <el-col :span="21" style="padding-bottom: 20px">
                <div
                    style="border:1px #DCDFE6 solid; height: 100%;border-radius: 4px ;width: 100% ;margin: 20px"
                >
                    <el-form
                        :model="request"
                        :rules="rules"
            ...
            <el-col :span="3" class="ms-left-cell" v-if="showScript">
                <el-button class="ms-left-buttion" size="small" @click="addPre"
                    >+{{ $t('api_test.definition.request.pre_script') }}</el-button
                >
                <br />
                <el-button class="ms-left-buttion" size="small" @click="addPost"
                    >+{{ $t('api_test.definition.request.post_script') }}</el-button
                >
                <br />
                <el-button class="ms-left-buttion" size="small" @click="addAssertions"
                    >+{{ $t('api_test.definition.request.assertions_rule') }}</el-button
                >
                <br />
                <el-button class="ms-left-buttion" size="small" @click="addExtract"
                    >+{{ $t('api_test.definition.request.extract_param') }}</el-button
                >
            </el-col>
        </el-row>
...
export default {
    ...
    data() {
        var checkdataSourceId = (rule, value, callback) => {
            if (!this.request.dataSourceId) {
                callback(new Error(this.$t('api_test.request.sql.dataSource_cannot_be_empty')));
            } else {
                callback();
            }
        };
        return {
            ...
            rules: {
                dataSourceId: [{ required: true, validator: checkdataSourceId, trigger: 'blur' }],
            },
        };
    },
    watch: {
    ...
    methods: {
        changeDataSource() {
            this.reload();
            this.$refs['request'].validateField('dataSourceId');
        },
        addPre() {
...
.ms-left-buttion:nth-of-type(1) {
    color: #0757cd;
    background-color: #ddebff;
    border: #0757cd;
}

.ms-left-buttion:nth-of-type(2) {
    color: #f09e3e;
    background-color: #ffe9cf;
    border: #f09e3e;
}

.ms-left-buttion:nth-of-type(3) {
    color: #3eaf8b;
    background-color: #dafde0;
    border: #dafde0;
}

.ms-left-buttion:nth-of-type(4) {
    color: #656af9;
