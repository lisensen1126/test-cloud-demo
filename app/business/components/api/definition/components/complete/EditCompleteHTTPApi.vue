        ...
        <el-card class="card-content" v-loading="httpForm.loading">
            ...
            >
                ...
                <div class="base-info">
                    ...
                    <el-row>
                        <el-col :span="24">
                            <el-form-item :label="$t('commons.description')" prop="description" class="description">
                                <el-input
...
export default {
  ...
  methods: {
    getIsMock() {
      let url = `/project/${this.projectId}/settings/function/order?function=mock_test`
      const config = {
        method: 'get',
        url,
      ...
      this.$request(config, response => {
        this.isShowMock = response?.data ? JSON.parse(response.data) : false
      })
    },
    runTest() {
...
.base-info .el-form-item.description >>> .el-form-item__content {
  width: auto;
  margin-right: 20px;
}
