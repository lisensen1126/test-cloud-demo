<template>
  <div v-if="request.hashTree && request.hashTree.length > 0">
    ...
    <!-- HTTP 请求参数 -->
    <div style="height: 100%;width: 100%" v-loading="isReloadData" v-if="request.hashTree && request.hashTree.length>0">
      ...
      <div v-for="row in request.hashTree" :key="row.id">
        ...
        <!--后置脚本-->
        <ms-jsr233-processor 
          v-if="row.label ==='JSR223 PostProcessor'" 
          @copyRow="copyRow" 
          @remove="remove" 
          :is-read-only="false" 
          :title="$t('api_test.definition.request.post_script')"
          :jsr223-processor="row" color="#F09E3E" 
