<template>
  <ms-container v-if="renderComponent" v-loading="loading" class="wrapper">
    <!-- 左侧操作栏 -->
    <ms-aside-container>
    ...
    <!-- 右侧 -->
    <ms-main-container>
      <el-tabs v-model="activeName" class="tab-main" @tab-click="addTab" @tab-remove="removeTab">
        <el-tab-pane name="title" disabled>
            <span slot="label" class="tab-title">{{ $t('i18n.automation')}} </span>
        </el-tab-pane>
        ...
        <el-tab-pane name="add">
          <template v-slot:label>
            <el-dropdown @command="handleCommand" v-tester>
              <el-button type="primary" icon="el-icon-plus" size="mini" v-tester>
...
<style scoped>
  ...
  .wrapper /deep/ .el-tabs__nav {
    width: 100%;
  }
  .wrapper /deep/ .tab-title {
  ...
  .wrapper /deep/ #tab-add{
    position: absolute;
    right: 0;
  }
  .wrapper /deep/ .el-tabs__item.is-disabled {
    color: #202D40;
  }

  .wrapper /deep/ .el-tabs__item.is-active {
    color: #3385ff;
  }
</style>
