<template>
    <div class="card-content">
            <div class="ms-main-div" @click="showAll" v-if="type !== 'detail'">
                ...
                >
                        ...
                        <el-col :span="12">
                            <el-form-item :label="$t('commons.name')" prop="name">
                                <el-input
                                    ...
                                    size="small"
                                    v-model.trim="currentScenario.name"
                                />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                        ...
                        <el-col :span="12">
                            <el-form-item :label="$t('commons.status')" prop="status">
                                <el-select
                                    class="ms-scenario-input"
                                    size="small"
                                    v-model="currentScenario.status"
                                >
                                    <el-option
                    ...
                    <el-row>
                        ...
                        <el-col :span="12">
                            <el-form-item
                                :label="$t('api_test.automation.follow_people')"
                                prop="followPeople"
                            >
                                <el-select
                                    v-model="currentScenario.followPeople"
                                    :placeholder="$t('api_test.automation.follow_people')"
                                    filterable
                                    size="small"
                                    class="ms-scenario-input"
                                >
            ...
            <div v-loading="loading">
                <div @click="showAll">
                    <p class="tip border-none">{{ $t('api_test.automation.scenario_step') }}</p>
                </div>
                <el-row>
                    <el-col :span="21">
                        <!-- 调试部分 -->
                        <div class="ms-debug-div" @click="showAll">
                            <el-row style="margin: 5px">
                                <el-col :span="4" class="ms-col-one ms-font">
                                    <el-tooltip placement="top" effect="light">
                                        ...
                                        <span class="scenario-name">
                                            {{
                                                currentScenario.name === undefined || ''
                                ...
                                <el-col :span="4" class="ms-col-one ms-font">
                                    <el-link class="head" :underline="false" @click="showScenarioParameters">{{
                                ...
                                <el-col :span="5">
                                    <el-button
                                        ...
                                        v-prevent-re-click
                                        @click="runDebug"
                                        >{{ $t('api_test.request.debug') }}</el-button
                                    >
                                    <el-tooltip
                                        class="item"
                                        effect="dark"
                        ...
                        <div>
                            <el-button
                                class="el-icon-files ms-open-btn ms-open-btn-left"
                                size="mini"
                                v-prevent-re-click
                                @click="openExpansion"
                            >
                            ...
                            <el-button
                                class=" el-icon-notebook-1 ms-open-btn"
                                size="mini"
    ...
    methods: {
        ...
        addComponent(type) {
            switch (type) {
                ...
                case ELEMENT_TYPE.Assertions:
                    this.selectedTreeNode != undefined
                        ? this.selectedTreeNode.hashTree.push(new Assertions())
                        : this.scenarioDefinition.push(new Assertions());
                    break;
                case ELEMENT_TYPE.Extract:
                    this.selectedTreeNode != undefined
                        ? this.selectedTreeNode.hashTree.push(new Extract())
                ...
                case ELEMENT_TYPE.LoopController:
                    this.selectedTreeNode != undefined
                        ? this.selectedTreeNode.hashTree.push(new LoopController())
        ...
        sort() {
            for (let i in this.scenarioDefinition) {
                ...
                // 设置循环控制
                if (
                    this.scenarioDefinition[i].type === ELEMENT_TYPE.LoopController &&
                ...
                if (!this.scenarioDefinition[i].projectId) {
                    ...
                    const project = this.projectList.find(
                        p => p.id === this.scenarioDefinition[i].projectId
                    );
                    if (!project) {
                        this.scenarioDefinition[i].projectId = this.projectId;
                    }
                ...
                if (
                    this.scenarioDefinition[i].hashTree != undefined &&
                    this.scenarioDefinition[i].hashTree.length > 0
                ) {
                    this.recursiveSorting(
                        this.scenarioDefinition[i].hashTree,
                        this.scenarioDefinition[i].projectId
                    );
                }
                // 添加debug结果
                if (this.debugResult && this.debugResult.get(this.scenarioDefinition[i].id)) {
                    this.scenarioDefinition[i].requestResult = this.debugResult.get(
                        this.scenarioDefinition[i].id
                    );
                }
            }
        },
        ...
        remove(row, node) {
            ...
            this.$alert(
                this.$t('api_test.definition.request.delete_confirm_step') + ' ' + name + ' ？',
                '',
                {
        ...
        getApiScenario() {
            ...
            if (
                ...
                !(Array.isArray(this.currentScenario.tags))
            ) {
                this.currentScenario.tags = this.currentScenario.tags ? JSON.parse(this.currentScenario.tags) : [];
            }
            if (!this.currentScenario.variables) {
            ...
            if (this.currentScenario.id) {
                this.result = this.$get(
                    '/api/automation/getApiScenario/' + this.currentScenario.id,
                    response => {
                        if (response.data) {
                            this.path = '/api/automation/update';
        ...
        setParameter() {
            ...
            if (this.currentModule != null) {
                this.currentScenario.modulePath =
                    this.currentModule.method !== undefined ? this.currentModule.method : null;
...
.card-content /deep/ .el-tree-node__content {
    height: 100%;
    margin-top: 3px;
...
.card-content >>> .el-drawer__body {
    overflow: auto;
}

.card-content /deep/ .el-step__icon.is-text {
    border: 1px solid;
}

.card-content /deep/ .el-drawer__header {
    margin-bottom: 0px;
}

.card-content /deep/ .el-link {
    font-weight: normal;
}

.card-content /deep/ .el-checkbox {
    color: #303133;
    font-family: 'Helvetica Neue', Helvetica, 'PingFang SC', 'Hiragino Sans GB', Arial, sans-serif;
    font-size: 13px;
    font-weight: normal;
...
.card-content /deep/ .el-checkbox__label {
    padding-left: 5px;
}
...
.card-content .ms-tree {
    width: 90%;
    margin-right: 10px;
}
.card-content .ms-tree >>> .el-tree-node__expand-icon.expanded {
    -webkit-transform: rotate(0deg);
    transform: rotate(0deg);
}

.card-content .ms-tree >>> .el-icon-caret-right:before {
    content: '\e723';
    font-size: 20px;
}

.card-content .ms-tree >>> .el-tree-node__expand-icon.is-leaf {
    color: transparent;
}

.card-content .ms-tree >>> .el-tree-node__expand-icon {
    color: #7c3985;
}

.card-content .ms-tree >>> .el-tree-node__expand-icon.expanded.el-icon-caret-right:before {
    color: #7c3985;
    content: '\e722';
    font-size: 20px;
}

.card-content .ms-sc-variable-header >>> .el-dialog__body {
    padding: 0px 20px;
}
