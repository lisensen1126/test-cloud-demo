<template>
  <div v-loading="result.loading">
    <div v-for="(pe, i) in data" :key="pe.id" style="margin-bottom: 20px;">
      <el-select ref="selectEnv" v-model="pe['selectEnv']" placeholder="请选择环境" style="margin-top: 8px;width: 200px;" size="small">
        <el-option v-for="(environment, index) in pe.envs" :key="index"
                   :label="environment.name"
                   :value="environment.id"/>
        <el-button class="ms-scenario-button" size="mini" type="primary"
                   @click="openEnvironmentConfig(pe.id, pe['selectEnv'], i)">
...
export default {
  ...
  methods: {
    ...
    openEnvironmentConfig(projectId, envId, i) {
      if (!projectId) {
