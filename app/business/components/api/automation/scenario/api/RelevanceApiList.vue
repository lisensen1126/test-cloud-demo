<template>
    <div>
        ...
        >
            ...
            <el-input
                :placeholder="$t('commons.search_by_name_or_id')"
                @blur="initTable"
                class="search-input"
                style=""
                size="small"
    ...
    methods: {
        ...
        initTable(projectId) {
            ...
            this.result = this.$post(
                ...
                response => {
                    ...
                    this.tableData = response.data.listObject.map(item => {
                        if (
                            item.tags &&
                            typeof item.tags == 'string' &&
                            typeof JSON.parse(item.tags) == 'object'
                        ) {
        ...
        genProtocalFilter(protocalType) {
            if (protocalType === 'HTTP') {
                ...
                this.methodFilters = [{ text: 'TCP', value: 'TCP' }];
            } else if (protocalType === 'SQL') {
                this.methodFilters = [{ text: 'SQL', value: 'SQL' }];
...
.api-table >>> .ms-table .el-table__body-wrapper {
