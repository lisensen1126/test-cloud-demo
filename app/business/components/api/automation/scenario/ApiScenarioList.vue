    ...
    <div>
        <el-card class="table-card-nopadding" v-loading="result.loading">
            ...
            <el-table
                ...
                border
                :data="tableData"
                class="adjust-table ms-select-all-fixed"
                @sort-change="sort"
            ...
            >
                ...
                >
                    <template v-slot:default="scope">
                        <show-more-btn
                            ...
                            :buttons="trashEnable ? trashButtons : buttons"
                ...
                <template v-for="(item, index) in tableLabel">
                    ...
                    <el-table-column
                        ...
                        prop="userName"
                        min-width="120px"
                        :label="$t('api_test.automation.principal')"
                        :filters="userFilters"
                ...
                >
                    ...
                    <template v-slot:default="{ row }">
                        ...
                        <div v-else>
                            <ms-table-operator-button
                                :tip="$t('api_test.automation.edit')"
                                icon="el-icon-edit"
                                @exec="edit(row)"
                                v-tester
                            />
...
export default {
    ...
    methods: {
        ...
        remove(row) {
            ...
            this.$alert(
                this.$t('api_test.definition.request.delete_confirm') + ' ' + row.name + ' ？',
                '',
                {
                    confirmButtonText: this.$t('commons.confirm'),
                    callback: action => {
        ...
        exportApi() {
            ...
            this.result = this.$post('/api/automation/export', param, response => {
                ...
                downloadFile(
                    'Metersphere_Scenario_' + localStorage.getItem(PROJECT_NAME) + '.json',
                    JSON.stringify(obj)
                );
            });
        },
