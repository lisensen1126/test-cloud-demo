        ...
        >
            <template v-slot:behindHeaderLeft>
                <el-tag
                    ...
                    type="danger"
                    >{{ $t('api_test.automation.reference_deleted') }}</el-tag
                >
                <el-tag size="mini" class="ms-tag" v-if="request.referenced === 'Copy'">{{
                    $t('commons.copy')
                }}</el-tag>
                ...
                <div v-if="request.result">
                    <el-tabs v-model="request.activeName" closable class="ms-tabs">
                        <el-tab-pane
                            :label="item.name"
                            :name="item.name"
                            v-for="(item, index) in request.result.scenarios"
                            :key="index"
                        >
                            <div
                                v-for="(result, i) in item.requestResults"
                                :key="i"
                                style="margin-bottom: 5px"
                            >
                                <api-response-component
                                    v-if="result.name === request.name"
                                    :result="result"
                                />
                            </div>
                        </el-tab-pane>
...
const esbDefinition =
    (requireComponent != null && requireComponent.keys().length) > 0
        ? requireComponent('./apidefinition/EsbDefinition.vue')
        : {};
const esbDefinitionResponse =
    (requireComponent != null && requireComponent.keys().length) > 0
        ? requireComponent('./apidefinition/EsbDefinitionResponse.vue')
        : {};
...
export default {
    ...
    created() {
        ...
        if (
            ...
            JSON.stringify(esbDefinition) != '{}' &&
            JSON.stringify(esbDefinitionResponse) != '{}'
        ) {
    ...
    methods: {
        ...
        getApiInfo() {
            if (this.request.id && this.request.referenced === 'REF') {
                ...
                let url =
                    this.request.refType && this.request.refType === 'CASE'
                        ? '/api/testcase/get/'
                        : '/api/definition/get/';
        ...
        sort() {
            for (let i in this.request.hashTree) {
                ...
                if (
                    this.request.hashTree[i].hashTree != undefined &&
                    this.request.hashTree[i].hashTree.length > 0
                ) {
