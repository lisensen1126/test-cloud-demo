<template>
  <el-card class="table-card" v-loading="result.loading">
    ...
    <el-table height="300" border :data="tableData" class="adjust-table table-content" @row-click="link">
      ...
      <el-table-column width="150" :label="$t('schedule.next_execution_time')">
        <template v-slot:default="scope">
          <crontab-result :enable-simple-mode="true" :ex="scope.row.value" ref="crontabResult"/>
        </template>
      </el-table-column>
      <el-table-column width="150" :label="$t('report.user_name')" prop="userName"/>
      <el-table-column width="150" :label="$t('commons.update_time')">
        <template v-slot:default="scope">
